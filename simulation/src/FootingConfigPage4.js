...
//            $("#MsgModal").html(modelImg2);
 ...
 $("#ptPerSubmit1").click(function(){
	ptPercentValy =  ((astProvideVal)/(bfConvert*effValRound)*100).toFixed(2);
	ptPercentValy = parseFloat(ptPercentValy);
	
	var ptPercentValyEnter = $("#ptPer1").val();
    
     if (ptPercentValyEnter == "") {
        $(".modal-header").html("Error Message");
        $(".modal-header").css("background", "#9c1203b0");
        $("#btnModal").removeClass("btn-success").addClass("btn-danger");
        $("#MsgModal").html("Provide the necessary value");
        ...
        ptPercentValyEnter = parseFloat($("#ptPer1").val());
	   
	   if (id21 <= 3) {
            if (ptPercentValyEnter == ptPercentValy) {
            ...
            if (ptPercentValyEnter == ptPercentValy) {
               $("#ptPerSubmit1").prop("disabled", true);
                ...
                $(".modal-header").html("Success Message");
                $(".modal-header").css("background", "#5cb85c");
                $("#MsgModal").html("Correct Answer is " + ptPercentValy);
            }
        }
        id21++;
